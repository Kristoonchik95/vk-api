...
## Описание задачи  
Мне нужно было реализовать отправку комментария к фотографии профиля ВКонтакте (например, [этот](https://vk.com/jump_tages)) .  
Требования:  
...
1. Перешла в [раздел для разработчиков](https://vk.com/apps?act=manage).  
...
## 📂 Что я положила в репозиторий
vk-api/  
├── postman/  
│   └── TZ.postman_collection.json  # Готовая коллекция для Postman
│   ├── screenshots/  
│   ├── comment.png         
...
## 🔐 Советы от меня  
- Никогда не публикуйте токен в коде! Даже если репозиторий приватный — перевыпустите токен сразу после тестов.  
- Проверяйте права токена: https://api.vk.com/method/account.getAppPermissions?access_token=ВАШ_ТОКЕН&v=5.131
    Если в ответе нет 4 (право photos) — запросы к комментариям не сработают.  
- Фото-аватарки могут блокировать комментарии. Проверьте настройки приватности владельца фото.  


И еще, в моменте, решила попробовать один способ публикации коментария (в общем читерство🙈). Уверена, Вы догадываетесь о чем я, но если вдруг хотите убедиться- расскажу))
